...
integer :: k,l,m,n,iset, j,iy,k_ppE
real(dp) :: m1, m2, mtotal, mc, eta, tsolar=4.925491_dp*10.0_dp**(-6), &
        ...
        lambdag,const,SNR_des,factor,sigma90,alpha,delta,psi,inc,GMST, &
        GMST_H, GMST_L, GMST_V, Omega_s, phi_delay_H, phi_delay_L,phi_delay_V, &
       Beta_PPE,Alpha_PPE,H0,SNR,sigma_vel,n_gal,Omega_s_68,dc_min,dc_max,Vc_min,Vc_max,n_host,dl_rescale,k_p,&
       b_PPE,n_ppE,a_ppE,gasdev,m10,m20,idum1,idum2,idum
...
do k_ppE=-10,6
n_ppE=(k_ppE/2.0_dp)+1.0_dp
b_ppE=2._dp*n_ppE-5._dp
a_ppE=2._dp*n_ppE


!Beta_PPE=0._dp !Fiducial value for beta
Alpha_PPE=0._dp !Fiducial value for alpha
   ...
   m1=35.6_dp*(1.0_dp+z)
    ...
    tc=1126259462.4158_dp  !Average gpstime (New LIGO posterior does not contain gpstime.This is taken from version 13. Since GMST in beam pattern function should be a function of     &tc,I need to write it in gpstime)
    !GMST=0._dp
    !tc=0._dp
    phic=0.0_dp

    !effective spin
    chi=0._dp !mean effective spin
    !chi=0._dp
...
open (unit=1,file='amp.txt')
open (unit=10,file='phase.txt')
open (unit=11,file='pn.txt')

! do loop for beta_ppE at different values of b_ppEs




!Calling the waveform function
call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_ppE,ad1,ad2,ad3,f3)
...
call fish(mtotal,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_ppE,fm,fmi)
...
write (1,*) sqrt(fmi(11,11))
!write(10,*)sqrt(fmi(11,11))
!write(11,*)n_ppE
end do
...
print *, "delta tc/tc=", sqrt(fmi(5,5))
...
!print *, "Beta_PPE=", sqrt(fmi(11,11))
print *, "Alpha_PPE=", sqrt(fmi(11,11))
...
subroutine fish(mtotal,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,fm,fmi)

use nrtype_qp
implicit none

!input parameters
real(dp), intent(in) :: mtotal,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE

!output parameters (fisher matrix "fm" and its inverse "fmi")
real(dp), dimension(imax,imax), intent(inout) :: fm,fmi
!integer, intent(in) ::b_PPE,a_ppE
integer :: i,j,n
...
call wave(f,mc*(1._dp+epsilon),eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
!This second one is for complex conjugate
call wave(f,mc*(1._dp-epsilon),eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
!Hangford LIGO
...
call wave(f,mc,eta*(1._dp+epsilon),chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta*(1._dp-epsilon),chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(2)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(2)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(2)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi+epsilon,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi-epsilon,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(3)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(3)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(3)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const*(1._dp+epsilon),tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const*(1._dp-epsilon),tc,phic,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(4)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(4)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(4)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc*(1._dp+epsilon),phic,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc*(1._dp-epsilon),phic,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(5)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(5)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(5)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc,phic+epsilon,alpha,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic-epsilon,alpha,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(6)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(6)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(6)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc,phic,alpha+epsilon,delta,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic,alpha-epsilon,delta,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(7)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(7)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(7)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc,phic,alpha,delta+epsilon,psi,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic,alpha,delta-epsilon,psi,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(8)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(8)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(8)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi+epsilon,inc,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi-epsilon,inc,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(9)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(9)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(9)=(adt3-adt_3)/(2.0_dp*epsilon)

call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc+epsilon,Alpha_PPE,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc-epsilon,Alpha_PPE,adt_1,adt_2,adt_3,f3)
dad1(10)=(adt1-adt_1)/(2.0_dp*epsilon)
dad2(10)=(adt2-adt_2)/(2.0_dp*epsilon)
dad3(10)=(adt3-adt_3)/(2.0_dp*epsilon)


call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE+epsilon,adt1,adt2,adt3,f3)
call wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE-epsilon,adt_1,adt_2,adt_3,f3)
...
subroutine wave(f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_ppE,ad1,ad2,ad3,f3)

use nrtype_qp
implicit none

!input binary & theory parameters
real(dp), intent(in) :: f,mc,eta,chi,const,tc,phic,alpha,delta,psi,inc,Alpha_PPE
...
gmst=-45954.219056772801676030_dp+0.000072888421391359328059975_dp*tc !Relation between GPStime and GMStime
!gmst=tc
!************************************************
...
wm=(1.0_dp+(Alpha_PPE*eta**(a_PPE/5._dp)*vf1**a_PPE)+alpha2*vf1**2+alpha3*vf1**3)/(1.0_dp+eps1*vf1+eps2*vf1**2)
...
if (f < f1) then
        amp = const*f**(-7.0_dp/6.0_dp)*(1.0_dp+Alpha_PPE*eta**(a_PPE/5._dp)*vf**a_PPE+alpha2*vf**2+alpha3*vf**3)
...
INTEGER :: IA=16807,IQ=127773,IR=2836,NDIV=1+(IM-1)/NTAB

REAL(DP), PARAMETER ::EPS=1.2q-7

REAL(DP) :: AM=1.q0/IM,RNMX=1.q0-EPS,idum,ran1
