...
Let us finish this section by explaining how we can utilize both amplitude and phase corrections to derive constraints on some theory. One can include $\alpha_{\PPE}$ and $\beta_{\PPE}$ as variables to the Fisher analysis as in Eq.~\eqref{parameters} and map them to a non-GR parameter of a theory to derive \sout{individual} constraints \kent{from the phase and amplitude independently}. We refer to such constraints as  the ``phase-only'' and ``amplitude-only'' bounds respectively. How can we achieve a single constraint that accommodates both of them? Recall the relations between the PPE parameters in Sec.~\ref{section:ppE}. One can rewrite $\alpha_{\PPE}$ in the waveform in terms of $\beta_{\PPE}$ according to Eqs.~\eqref{eq:2w2} or~\eqref{eq:2w3} and eliminate the former variable from the analysis. The bound derived from $\beta_{\PPE}$ now takes into account non-GR corrections to not only the phase but also the amplitude. \kent{We refer to such constraints as the ``phase \& amplitude combined'' bounds.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Results in Example Theories}

We now apply our analysis to some example theories.  \kent{To save computational time for our Monte-Carlo simulations,} we use \kent{IMRPhenomB} waveform. \kent{For corrections entering through propagation mechanisms, Ref.~\cite{Yunes:2016jcc} showed that the difference in constraints on PPE parameters between IMRPhenomB and IMRPhenomD waveforms are negligible. In App.~\ref{Appendix}, we perform a similar comparison for generation mechanism corrections using sky-averaged waveforms and show that the former is only} 
\sout{which is} suitable for constraining generation mechanisms that enter at negative PN orders. \sout{A comparison between PhenomB and PhenomD for such mechanisms is presented in App.~\ref{Appendix}.}
\kent{For this reason,} we consider below massive gravity, EdGB gravity, scalar-tensor theories and varying-$G$ theories. \sout{While in} Massive gravity \kent{acquires} non-GR corrections \sout{enters} through propagation mechanisms \kent{while} others achieve the corrections through generation mechanisms \kent{entering at negative PN orders}.

%Also, in massive gravity we focus on the non-GR correction in GW phase only. While for the others we discuss both phase and amplitude corrections.
