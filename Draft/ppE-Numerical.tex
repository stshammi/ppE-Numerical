...
\caption{Comparision of combined and phase-only analyses at different PN orders from GW150914 with a sky-averaged phenomB waveform. We show $\delta_{\beta}=\lb\beta_{\text{comb}}-\beta_{\text{phase}}\rb/\beta_{\text{phase}}$, where $\beta_{\text{phase}}$ and $\beta_{\text{comb}}$ are bounds on $\beta_{\PPE}$ from phase-only and combined analyses respectively. When $\delta_\beta$ is positive (negative), the combined analyses yield weaker (stronger) bounds than the phase-only ones.}
