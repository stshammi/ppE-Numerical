...
\ky{The first paragraph is great, but I'm not sure if we need this second paragraph...The focus of our paper is to derive current bounds including amplitude corrections, and not how such bounds will improve with future detectors. Given that most of things explained in this paragraph is not directly related to what we do in our paper, I'm more inclined to comment out this paragraph completely, sorry... (If we decide to keep this paragraph, we should also comment on other space detectors, like DECIGO, B-DECIGO, TianQin etc., that are also actively studied, but then it gets longer and longer...)}
...
\end{tabular}
\end{centering}
\caption{\kent{90\% credible} constraints on representative parameters of various modified theories of gravity from GW150914 and GW151226. For each of the GW events, ``phase'' and ``amplitude'' correspond to the cases where we include non-GR corrections only to the GW phase and amplitude respectively, while ``combined'' is the case where we include both. $\bar{\alpha}_{\EDGB}$ is the EdGB coupling parameter which is related to the dimensionless coupling $\zeta_{\EDGB}$ by $\zeta_\EDGB\equiv 16 \pi \bar{\alpha}_\EDGB^2/m^4$ with $m$ being the total mass of the binary. $m_1\dot{\phi}$ \kent{corresponds to a dimensionless parameter in the theory where $m_1$ is the mass of the primary BH while $\phi$ is the scalar field. Non-GR corrections are always subdominant whenever $\zeta_\EDGB \ll 1$ or $m_1\dot{\phi} \ll 1$.} 
\ky{I realized that the word ``small coupling'' is a bad phrasing for ST since $\dot \phi$ is not a coupling parameter of the theory...}
\sout{controls the amount of small coupling in scalar-tensor theory where $m_1$ is the larger binary mass component and $\phi$ is the scalar field.} $G$ is the gravitational constant with the subscript 0 representing the time of coalescence. An overhead dot denotes a derivative with respect to time.
\ky{I see inconsistency in number of digits...Sometimes, it only has 3 digits (like 0.0220) while it sometimes go up to 6 digits (136.643). I now think we should probably show everything up to 3 digits. I don't think our Fortran Fisher code is numerically accurate to 6 digits...In some cases, it's true that if we only show up to 3 digits, we don't get any difference between phase and combined, but that's what it is and I think that's fine. Also, I suggest to add brackets in bounds on EdGB and ST that don't satisfy the small coupling approx.}
}
\label{table:ppE}
\end{table*}
\endgroup
