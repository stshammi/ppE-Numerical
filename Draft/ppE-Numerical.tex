...
\st{I added the following paragraph}

Let us finish this section by explaining how we can utilize both amplitude and phase corrections to derive constraints on some theory. One can include $\alpha_{\PPE}$ and $\beta_{\PPE}$ as variables to the Fisher analysis as in Eq.~\eqref{parameters} and map them to the non-GR parameter of a theory to derive individual constraints. We refer to such constraints as  the phase-only and the amplitude-only bounds respectively. How can we achieve a single constraint that accommodate both of them? Recall the relations among the PPE parameters in Sec.~\ref{section:ppE}. One can rewrite $\alpha_{\PPE}$ in the waveform in terms of $\beta_{\PPE}$ according to Eqs.~\eqref{eq:2w2} or~\eqref{eq:2w3} and eliminate the former variable from the analysis. The bound derived from $\beta_{\PPE}$ now takes into account non-GR corrections to not only the phase but also the amplitude.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Results in Example Theories}
\st{I added the following paragraph. If it's not needed here we can move it to the introduction later.}

We now apply our analysis to some example theories. We consider massive gravity, EdGB gravity, scalar-tensory theory, and varying-$G$ theory. While in massive gravity the non-GR correction enters through the propagation mechanism, others achieve it through generation mechanisms. We use PhenomB waveform which is suitable for constraining generation mechanisms that enter at negative PN orders only. A comparison between PhenomB and PhenomD for such mechanisms is presented in App.~\ref{Appendix}.
