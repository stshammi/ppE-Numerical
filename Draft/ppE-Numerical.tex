...
%What we did
We derived constraints on scalar-tensor, EdGB and varying-$G$ theories from GW150914 and GW151226. To do so we performed Fisher analyses with Monte-Carlo simulations using a phenomB waveform.
%We focused on theories where the leading correction enters at a negative PN order.
In particular, we derived reliable constraints on scalar-tensor theory from GW observations for the first time.
%massive gravity?
...
We found that for binaries with large masses such as GW150914 where we have less number of cycles the bounds from amplitude and phase can be comparable to each other.
...
On the other hand, combined analyses yield constraints that differ from the phase-only case at most by 3.6\% for the theories under consideration. Hence at least in the theories where the leading correction enters at a negative PN order the phase correction only can produce a sufficiently accurate constraint.
\begin{figure}[htb]
\includegraphics[width=8.5cm]{combinedvsphase.pdf}
\caption{Comparision of combined and phase-only analyses at different PN orders from GW150914 with a sky-averaged waveform. $\delta_{\beta}=\lb\beta_{\text{comb}}-\beta_{\text{phase}}\rb/\beta_{\text{phase}}$, where $\beta_{\text{phase}}$ and $\beta_{\text{comb}}$ are the $\beta_{\PPE}$ from phase-only and combined analysis respectively. }
\label{fig:combinedvsphase}
\end{figure}

%whether combined analyses give better bound
Depending on the prior information and the PN order of the non-GR correction a combined analysis can yield stronger or weaker constraint compared to a phase-only one. With the priors mentioned in Sec.~\ref{data} the fractional difference between the $\beta_{\PPE}$'s of the two cases is presented in Fig.~\ref{fig:combinedvsphase}. From -4 PN to -2.5 PN the combined analyses produce slight improvements over the phase-only constraints, while otherwise the former is weaker with a maximum deterioration of 8.5\% at -1 PN. Nonetheless, combined analyses are more reliable since excluding amplitude corrections from the waveform causes more uncertainty in the luminosity distance.
