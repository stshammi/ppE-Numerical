...
\kent{In this section, we explain how we perform our analysis. We first explain the PPE formalism and non-GR waveform templates. We then describe the Fisher analysis and how we construct probability distributions of non-GR parameters.}
...
\end{equation}
where $\tilde{h}_{\GR}$ is the gravitational waveform in GR. \sout{$\delta\Psi$ corresponds to the non-GR correction to the GW phase while} $\alpha_{\PPE}\, u^a$ \kent{corresponds to the non-GR correction} to the GW amplitude with $u\equiv(\pi \mathcal{M} f)^{1/3}$.  $\mathcal{M}\equiv(m_1m_2)^{3/5}/(m_1+m_2)^{1/5}$ is the chirp mass with component masses $m_1$ and $m_2$ and $f$ is the frequency of the GW. \kent{The constant $\alpha_{\PPE}$ controls the overall magnitude of the correction while the index $a$ specifies at which PN order the correction enters.} One can write the \kent{non-GR} phase correction \kent{$\delta\Psi$} in a similar manner as that of the amplitude as
...
%Parametrization
PPE modifications in Eq.~\eqref{eq:2a} can enter through the non-GR corrections to the binding energy and the GW luminosity~\cite{Yunes:2009ke,Chatziioannou:2012rf}, or alternatively to the frequency evolution and the Kepler's law~\cite{Tahura:2018zuq}. We \sout{are going to} \kent{will} follow the latter approach and write the modified Kepler's law as
\begin{equation}
 \label{eq:2c}
...
\end{equation}
Here, $\lb\gamma_r,c_r\rb$ and $\lb\gamma_{\dot{f}},c_{\dot{f}}\rb$ parametrize the non-GR corrections to the binary separation $r$ and the frequency evolution $\dot{f}$ respectively. 
\kent{To leading PN order, the GR contribution is given by~\cite{cutlerflanagan,Blanchet:1995ez}
\begin{equation}
r_{\GR}=\left(\frac{m}{\Omega^2}\right)^{1/3}\,, \quad
\dot{f}_{\GR}=\frac{96}{5}\pi^{8/3}\mathcal{M}^{5/3}f^{11/3}\,,
\end{equation}
}
\sout{$r_{\GR}$ is given by the Newtonian Kepler's law as $r_{\GR}=\left(m/\Omega^2\right)^{1/3}$,} where $m$ represents the total mass of the binary and $\Omega=\pi f$ is the orbital angular frequency. \sout{$\dot{f}_{\GR}$ is the frequency evolution in GR and to the leading PN order, it is given by $\dot{f}_{\GR}=\frac{96}{5}\pi^{8/3}\mathcal{M}^{5/3}f^{11/3}$.}

%Amplitude and phase correction
Utilizing the stationary phase approximation~\cite{PhysRevD.62.084036,Yunes:2009yz} and the quadrupole formula for the metric perturbation~\cite{Blanchet:2002av}, one can easily derive the amplitude and phase of the dominant quadrupolar mode in Fourier space from  \kent{Eqs.}~\eqref{eq:2c} and \sout{Eq.}~\eqref{eq:2d} as
...
\ee
respectively. Eq.~\eqref{eq:Psi} is already in the PPE format, while  Eq.~\eqref{eq:amp} can be reduced to such a form by \sout{knowing at which PN orders the dissipative and the conservative corrections enter and hence by choosing the dominant one} \kent{keeping only the dominant correction}\footnote{A detailed derivation can be found in Ref.~\cite{Tahura:2018zuq}}.\st{somewhere we have to give the definitions of dissipative and conservative corrections}


%Relation among ppE parameters
\kent{In fact, }
\sout{We now show the relations among} the PPE phase and the amplitude parameters \kent{may be related as follows}. 
...
\end{equation}
while for the conservative-dominanted case, we obtain
\begin{equation}\label{eq:2w3}
\alpha_{\PPE} =\frac{8}{15} \frac{(8-a)(5-a)(a^2-4a-6)}{a^2-2a-6} \beta_{\PPE}\,.
\end{equation}
On the other hand, when the aforementioned corrections enter at the same PN order, no direct relation between $\alpha_{\PPE}$ and $\beta_{\PPE}$ exists. The exponents \sout{corrections} $a$ and $b$ \kent{in the correction terms} are related by the following equation which is valid for all three cases:
\begin{equation}
b=a-5\,.
\end{equation}
...
\ky{I'd bring massive gravity first since this one is special. It doesn't involve amplitude corrections and the modification comes from propagation mechanisms while all the remaining theories involve generation mechanism modifications.}

\ky{I'd bring ST after EdGB since both corrections enter at $-1$PN, plus PPE for varying-G theories are a bit special.}
