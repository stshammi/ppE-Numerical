...
%%prior on spin
\st{We impose the prior information such that $-1 \leq \chi \leq 1$, $-\pi \leq (\phi_0, \alpha, \psi) \leq \pi$, and $-\pi/2 \leq ( \delta, \theta_{\text{inc}}) \leq \pi/2$.}
...
%Introduction
Many metric theories of gravity that violate strong equivalence principle~\cite{DiCasola:2013iia,Will:2014kxa,0264-9381-7-10-007} predicts time variation in the gravitational coupling parameter $G$~\cite{uzan:2010pm}. Scalar-tensor theories are examples where $G$ varies as a function of the time derivative of the asymptotic scalar field~\cite{Will2006}. Any such time-dependence of $G$ leads to a variation in the effective masses of the compact bodies, which in turn makes them experience anomalous cosmic acceleration~\cite{PhysRevLett.65.953}. Such phenomena alters the gravitational waveform through the modifications of the binary orbital period evolution and the energy balance law~\cite{Tahura:2018zuq}.

%PPE parameters
We now show the PPE modifications due to a time variation in the gravitational constant. The constants that appear in different sectors of a  gravitational theory may not be unique. Einstein-\AE ther theory~\citep{Yagi:2013ava} and Brans-Dicke theory with a cosmologically evolving scalar field~\cite{Will2006} are examples of such theories.
Ref.~\citep{Tahura:2018zuq} studies a generic case with two distinct gravitational constants in Kepler's law and GW luminosity. Here we place constraint on the special case where those two constants are same. Let the masses and the Newton's constant vary according to the following equations:
...
\label{eq:3.7d2}
 ...
 \end{eqnarray}
with $a=-8$ respectively. Here $s_A$ is the sensitivity of the Ath binary component.
 
%Results
Employing Eqs.~\eqref{eq:beta_gdot} and~\eqref{eq:alpha_gdot} GW150914 (GW151226) imposes constraints on $|\dot{G_0}/G_0|$ from the phase-only and the amplitude-only analyses as $1.2\times10^7\, \mathrm{yr}^{-1}$ ($3.6\times10^5\, \mathrm{yr}^{-1}$) and $1.8\times10^8\, \mathrm{yr}^{-1}$ ($6\times10^7\, \mathrm{yr}^{-1}$) respectively, with the combined analyses producing slight improvements over the phase-only outcomes.
These bounds are much less stringent compared to the other contemporary constraints~\cite{Will2006}. 
However, future space-borne detectors such as  eLISA~\cite{Seoane:2013qna} and DECIGO~\cite{Seto:2001qf} will be able to obtain constraints up to 13 orders of magnitude stronger compared to  the aLIGO ones~\cite{Chamberlain:2017fjl}.
