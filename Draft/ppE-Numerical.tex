...
The detection of gravitational waves \zack{has} offered us the opportunity to explore the dynamical and strong-field regime of gravity.
%Why amplitude correction is important
\sout{Since} \zack{Because} matched filtering is more sensitive to \sout{the} variation\zack{s} in the GW phase than the amplitude, many tests of gravity with gravitational waves have been carried out using only the former. Such studies cannot probe the non-GR effects that may enter \zack{only} in the amplitude \sout{only}. Besides, if not accommodated in the waveform template\zack{,} a non-GR effect in the amplitude may induce \zack{``induce" works but I think ``introduce" sounds better.} systematic errors on parameters \sout{that} \zack{which} are present in GR.
%PPE formalism
In this paper we derive constraints on some modified theories of gravity\zack{,} incorporating both \sout{the} phase and \sout{the} amplitude corrections. We follow the model-independent approach of \zack{the} \sout{parametrized} \zack{parameterized} post-Einsteinian formalism.
%What we do
We perform Fisher analyses with Monte-Carlo simulations focusing on non-GR corrections \zack{entering the waveform} at negative post-Newtonian orders \sout{originated} \zack{originating} from generation mechanisms.
%time-evolution of scalar field
In particular, we derive reliable constraints on the time-evolution of a scalar field \sout{with} in \zack{a} scalar-tensor theory for the first time with gravitational waves.
%three constraints
We derive constraints by treating \zack{the} phase and amplitude corrections independently\zack{,} and also by combining the\zack{m} both.
...
%conclusion
Also, constraints derived by incorporating both phase and amplitude \zack{corrections} differ from the ones \sout{from the} \zack{with} phase \zack{corrections only} \sout{at most by 4\%} \zack{by 4\% at most}, suggesting that studying only the phase is sufficient for non-GR generation effects \sout{which} enter\zack{ing} \sout{in} the waveform at negative post-Newtonian orders.
...
One can adopt either a model-independent\zack{,} or a theory-specific method for testing gravity, although the former is more efficient if one wishes to achieve constraints on multiple theories with \sout{gravitaional wave} GW\zc{already defined this acronym.} observations. To be theory-agnostic a data analysis pipeline \sout{called} \zack{named} TIGER \sout{was} \zack{has been} developed\zack{,} where one introduces \sout{parametrized} \zack{parameterized} deformations at various PN orders and treats them independently~\cite{Agathos:2013upa,Meidam:2014jpa}. With a similar goal\zack{,} the LIGO \sout{scientific collaboration} and VIRGO scientific collaboration (LVC) employed the generalized IMRphenom (gIMR) waveform \zack{model,} which \zack{has} included \sout{the} spin-precession effects \zack{as} well~\cite{Monitor:2017mdv,Abbott:2016izl}. With such waveforms\zack{,} tests of gravity with \zack{the} GW phase have been carried out in~\cite{Monitor:2017mdv,TheLIGOScientific:2016pea,Yunes:2016jcc,Abbott:2017vtc,LIGOScientific:2019fpa,TheLIGOScientific:2016src}. Both TIGER and gIMR have the drawback that they do not take into account the possible non-GR deviations in the GW amplitude. Moreover, they cannot capture the non-GR effects entering at negative PN orders such as the scalar-dipole radiation. Another framework proposed by Yunes \& Pretorius \sout{called} \zack{named the} \emph{parameterized post-Einsteinian} (PPE) formalism circumvents such limitations by introducing generic corrections to both the phase and the amplitude~\cite{Yunes:2009ke,Chatziioannou:2012rf}.
...
Almost all of the templates \zack{used} for matched filtering are based on the restricted post-Newtonian approximation~\cite{VanDenBroeck:2006qu}, which led many of the previous studies to focus only on the phase. Nonetheless, scenarios where amplitude corrections bear importance  are not uncommon. In some parity-violating theories\zack{,} one of the circularly-polarized mode\zack{s} is amplified while the other one is suppressed, \sout{which is} \zack{an effect} called amplitude birefrigence~\cite{Alexander:2007kv,Yunes:2008bu,Yunes:2010yf,Yagi:2017zhb}. Such an effect enters only in the GW amplitude and cannot be revealed by studying the phase. Probing amplitude corrections is also important in constraining gravitational theories with GW stochastic backgrounds~\cite{Maselli:2016ekw}. Furthermore, theories with flat extra dimensions~\cite{Cardoso:2002pa}, Horndeski gravity~\cite{Saltas:2014dha}, and f(R) gravity~\cite{Hwang:1996xh} may predict amplitude damping that scales with the cosmological distance. Such phenomena have been studied in Ref.~\cite{Nishizawa:2017nef}  in terms of a generalized GW propagation (gGP) framework. 



%our work:methodology
Motivated by the propagation effects in the GW amplitude, we study the generation \sout{effects} \zack{mechanisms} in this paper. PPE amplitude corrections due to generation mechanisms in various example theories have been derived analytically in Reference~\cite{Tahura:2018zuq}. We compute the constraints on some of those theories from both the phase and \sout{the} amplitude, focusing on leading PN corrections to the tensorial modes only. We choose theories where the leading correction enters at a negative PN order\zack{,} and the sensitivities of BHs are known. Such criteria lead us to choose \zack{Einstein-Dilaton-Gauss-Bonnet (EdGB)} \sout{EdGB}, scalar-tensor, and varying-$G$ gravit\zack{ies}. We carry out Fisher analyses with Monte-Carlo simulations utilizing the parameter estimation samples of GW151226 and GW150914 released by LIGO~\cite{ligo:sample}. Such analyses with actual posterior samples produce more reliable results compared to the ones with sky-averaged waveforms. In fact, \zack{when} implementing such samples\zack{,} we can determine the credibility of the small coupling approximation in scalar-tensor theory, which allows us to place reliable bounds on the time-evolution of the scalar field from GW observations for the first time.
...
We also find that the constraints derived from the phase and the amplitude can be comparable in case of massive binary systems like GW150914. Whereas for less massive \sout{ones} \zack{binaries} with \zack{a} large number of GW cycles\zack{,} the phase always yields stronger constraints. Moreover, inclusion of an amplitude correction to the waveform impacts the bound on the phase correction as well.  Such effects vary with the PN order of the corrections\zack{,} and the prior information \sout{of} \zack{used in} the analyses. %The constraints on various theories from is presented in Table~\ref{table:ppE}.
One can obtain a more definitive constraint by combining the ones from the phase and the amplitude. All such constraints in the theories under consideration is presented in Table~\ref{table:ppE}.
...
In this section, we explain how we perform our analysis. We first explain the PPE formalism and \zack{the} non-GR waveform template. We then describe the Fisher analysis and how we construct probability distributions of non-GR parameters.
...
\end{equation}
where $\tilde{h}_{\GR}$ is the gravitational waveform in GR. $\alpha_{\PPE}\, u^a$ \zack{is a correction} to the GW amplitude with $u\equiv(\pi \mathcal{M} f)^{1/3}$\zack{,} $\mathcal{M}\equiv(m_1m_2)^{3/5}/(m_1+m_2)^{1/5}$ is the chirp mass with component masses $m_1$ and $m_2$\zack{,} and $f$ is the frequency of the GW. The constant $\alpha_{\PPE}$ controls the overall magnitude of the correction\zack{,} while the index $a$ specifies at which PN order the correction enters. One can write the non-GR phase correction $\delta\Psi$ in a similar manner as that of the amplitude as
...
\end{equation}
while for the conservative-\sout{dominanted} \zack{dominated} case, we obtain\sout{s}
...
\ea
\ew
\sout{Here,} \zack{where} $\mathcal{M}_z$ is the the redshifted chirp mass\zack{,} and $\chi$ is the effective spin parameter~\footnote{The effective spin parameter is defined as $\chi\equiv\lb m_1 \chi_1+m_2\chi_2\rb /\lb m_1+m_2\rb$, where $\chi_A$ with $A=(1,2)$ is the dimensionless spin of the $A$th body.}. $\alpha, \delta, \psi$, and  $\theta_{\text{inc}}$ are the right ascension, declination, polarization and inclination angles respectively in the detector frame.
...
%%prior on spin
We impose \sout{the} prior information such that $-1 \leq \chi \leq 1$, $-\pi \leq (\phi_0, \alpha, \psi) \leq \pi$, and $-\pi/2 \leq ( \delta, \theta_{\text{inc}}) \leq \pi/2$.

%Detectors
We use the detector sensitivity of Advanced LIGO (aLIGO) O1 run~\cite{LIGOScientific:2018mvr}\zack{,} and we consider the two detectors at Hanford and Livingston\sout{e}. For simplicity, we assume that the Livingston\sout{e} noise spectrum is identical to that of \sout{the} Hanford~\cite{Yunes:2009yz}. For \zack{the} Fisher integration, the minimum frequency is taken to be 20 Hz while the maximum frequency is same as the cutoff frequency above which the signal power is negligible~\cite{Ajith:2009bn}.


%The rule of compound probability
Now we are going to discuss how we \sout{achieve} \zack{compute} the probability distribution of a non-GR parameter from the output of a Fisher analysis with a Monte Carlo simulation. We set the  fiducial value of any non-GR parameter to be zero for our analysis. We perform the following integration numerically to obtain the compound probability density function~\footnote{If the distribution of a random variable $y$ depends on a parameter $x$, and if $x$ is \sout{not fixed rather a random variable} \zack{a random variable, hence not fixed}\zack{I think this is what you mean here? I was confused by the original sentence.}, the marginal distribution of $y$ is called mixture distribution or compound probability distribution and is given by $P\left(y\right)=\int P\left(y|x\right) P\left(x\right)dx$. The underlying distribution $P\left(x\right)$ is called \zack{the} mixing \sout{distribution} or latent distribution.~\cite{2016arXiv160204060R}} of any parameter $\xi$~:
\be
\label{eq3:1}
...
%PPE parameters
Certain scalar-tensor theories predict scalarization of NSs~\cite{PhysRevLett.70.2220,Barausse:2012da}, which can also happen \st{for} \zack{to} BHs if the scalar field evolves with time cosmologically~\cite{Jacobson:1999vr,Horbatsch:2011ye}. Compact binaries formed by such objects emit dipole radiation which modifies the GW phase with~\cite{Freire:2012mg,Wex:2014nva,Tahura:2018zuq}
\be\label{eq:betaST}
...
\label{eq:3.7d2}
 ...
 \end{eqnarray}
with $a=-8$ respectively. Here $s_A$ is the sensitivity of the $A$th binary component\zc{Maybe give a reference or footnote on how to compute $s_A$?}.
...
%What we did
\zack{In this analysis, we have} \sout{We} derived constraints on scalar-tensor, EdGB and varying-$G$ theories from GW150914 and GW151226. To do so\zack{,} we performed Fisher analyses with Monte-Carlo simulations using the PhenomB waveform.
...
%Amplitude correction
We explored how amplitude corrections contribute to the constraints on such theories. We derived three set\zack{s} of bounds on each theory: phase-only, amplitude-only, and \zack{from both phase and amplitude} combined. We found that for binaries with large masses\sout{,} such as GW150914\zack{,} where we have less number of cycles, the bounds from the amplitude and phase can be comparable to each other. On the other hand, combined analyses yield constraints that differ from the phase-only case at most by 3.6\% for the theories under consideration. Hence\zack{,} at least in theories where the leading corrections enter at negative PN orders, the phase-only corrections can produce sufficiently accurate constraints.
\begin{figure}[htb]
