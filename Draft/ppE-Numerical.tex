...
%Introduction
Despite the fact that PhenomD waveform produces more accurate results, we utilized PhenomB  in this paper since the latter is simpler and time-saving when it comes to Monte Carlo simulations. It is only reasonable to ask the validity of such choice and one may want to justify our results against the PhenomD ones. Hence in this appendix we compare the constraints on PPE parameters from both waveforms. 
 
 %Comparison of waveforms
Let us discuss the distinct features of the two waveforms first. Both PhenomB and PhenomD are aligned-spin (non-precessing) frequency-domain phenomenological models of gravitational waveforms~\cite{Ajith:2009bn,Khan:2015jqa}. PhenomB is calibrated for mass ratios upto $m_1/m_2\equiv4$ and spin components of $\chi_i\in[-0.85,0.85]$. One the other hand, PhenomD covers a larger region of the parameter space with a mass ratio of 18 and spins of $\chi_i\in[-0.95,0.95]$. Moreover, PhenomD waveform amplitude and phase are continuous over the  inspiral, merger and  ringdown stages, while in PhenomB the continuity is ensured only in the amplitude. There is also an intermediate segment connecting the inspiral and the merger-ringdown parts in the former waveform. The spin-dependence in PhenomD is employed by two parameters, while PhenomB has one single effective spin parameter. The former also contains higher order GR terms which make such waveforms more reliable compared to the PhenomB ones.
...
\caption{Comparison of 90\% confidence constraint on $\alpha_{\PPE}$ with GW150914 from the the inspirals of PhenomB and PhenomD waveforms. The Fisher analyses are truncated at 104Hz which is the transition frequency between inspiral and merger of the PhenomB waveform.}
\label{fig:phenomBvsD-insp}
\end{figure}

%Amplitude corrction
Now we estimate the constraints on PPE amplitude modification from the two waveforms. Since the propagation effect entered only in massive gravity where we did not utilize the amplitude correction, focusing on the generation mechanism should suffice here. We perform Fisher analyses with sky-averaged waveforms and derive upper bounds on $\alpha_{\PPE}$ at different PN orders. As shown Fig.~\ref{fig:phenomBvsD}, the two waveforms agree very well at negative PN corrections but deviate from each other at the positive ones. Removing the higher order GR terms from PhenomD to match with that of PhenomB does not change the outcome, which eliminates such terms as sources of discrepancy. On the other hand, truncating the Fisher analyses at the end of inspiral phase show significant agreement between the two waveforms at $n>0$, suggesting a considerable amount of deviation coming from the intermediate-merger-ringdown portion (Fig.~\ref{fig:phenomBvsD-insp}).

%Conclusion
The example theories considered in this paper achieve leading non-GR corrections either from propagation effects or from the generation effects entering in negative PN orders. A comparison between PhenomB and PhenomD for constraining $\beta_{\PPE}$ presented in Ref~\citep{Yunes:2016jcc} reveal consistent results with propagation mechanisms at both positive and negative PN orders, while for generation mechanisms the two waveforms show agreement only at $n<0$. Together this with the nature of amplitude corrections discussed above confirm that the results of this paper should not change significantly if one utilizes PhenomD instead of PhenomB. One the other hand, for constraining theories like dynamic Chern-Simon or noncommutative gravity where the leading correction enters at $n>0$ PhenomB is not expected to produce reliable results.
